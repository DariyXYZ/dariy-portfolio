...
export function AnimatedIcon({ name, size = 18, className = "" }: { name: "arrow" | "external" | "download" | "chevron" | "close" | "menu" | "mail"; size?: number; className?: string }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false" className={[styles.icon, styles[name], className].join(" ")}>
    ...
    {name === "mail" ? <g className={styles.moving}><rect x="3" y="5" width="18" height="14" rx="3" /><path className={styles.flap} d="m3 7 9 6 9-6" /></g> : null}
